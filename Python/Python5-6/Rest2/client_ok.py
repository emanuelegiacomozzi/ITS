...
def EffettuaPrimoLogin():
    ...
    jRequest = {"username": sUsername, "password": sPassword}
    try:
        api_url = base_url +"/login"
        response = requests.post(api_url, json=jRequest, verify=False)
        if response.status_code == 200:
            jResponse = response.json()
            if jResponse["Esito"]=="000":
                sPrivilegio = jResponse["Privilegio"]
